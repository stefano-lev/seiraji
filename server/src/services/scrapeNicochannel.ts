import { getFanclubIdFromUrl } from './nicochannelResolver';

import { fetchNicochannelEpisodes } from './nicochannelEpisodes';
...
export async function scrapeNicochannel(url: string) {
  ...
  const [baseRes, episodes] = await Promise.all([
    fetch(
      `https://api.nicochannel.jp/fc/fanclub_sites/${fanclubId}/page_base_info`
    ),

    fetchNicochannelEpisodes(fanclubId),
  ]);
  ...
  return {
    ...
    episodes: normalizeEpisodes(
      filteredEpisodes.map((ep) => {
        ...
        return {
          id: `nicochannel:${ep.content_code}`,
